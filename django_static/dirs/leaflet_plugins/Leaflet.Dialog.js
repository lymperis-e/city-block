L.Control.Dialog = L.Control.extend({
  ...
  //Custom Event Listeners to emit destroyed signal directly, and not through the map
  ...
  close: function() {
    ...
    //Define a custom event to be fired directly from the dialog (not the map)
    
    let close_event = new Event("dialogClosed")
    this.domObj.dispatchEvent(close_event);
    //
  ...
  _initLayout: function() {
    ...
    var grabberIcon = L.DomUtil.create("i", "");
    grabberNode.appendChild(grabberIcon);

    
    //Replace the old way of dragging only by the grabber: Handle mousemove on all the dialog
    //L.DomEvent.on(grabberNode, "mousedown", this._handleMoveStart, this);
    L.DomEvent.on(this.getElement(), "mousedown", this._handleMoveStart, this);
    ...
    var resizeIcon = L.DomUtil.create("i", "fas fa-arrows-alt-h fa-rotate-45");
